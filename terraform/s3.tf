resource "random_string" "s3_unuque_key" {
  length  = 6
  upper   = false
  lower   = true
  special = false
...
resource "aws_s3_bucket" "s3_static_bucket" {
  bucket = "${var.project}-${var.environment}-static-bucket-${random_string.s3_unuque_key.result}"
...
resource "aws_s3_bucket" "s3_deploy_bucket" {
  bucket = "${var.project}-${var.environment}-deploy-bucket-${random_string.s3_unuque_key.result}"
